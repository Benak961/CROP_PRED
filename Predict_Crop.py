...
def main():
    html_temp_vis = """
    <div style="background-color:#025248 ;padding:10px;margin-bottom:30px">
    <h2 style="color:white;text-align:center;"> Visualize Soil Properties </h2>
    </div>
    """

    html_temp_pred = """
    <div style="background-color:#025280 ;padding:10px;margin-bottom:30px">
    ...
    if select_type == "Predict Your Crop":
        ...
        n = st.slider('NITROGEN', 0, 200)
        p = st.slider('PHOSPHORUS', 5, 140)
        k = st.slider('POTTASIUM', 5, 200)
        temperature = st.slider('TEMPERATURE', 8.83, 50.25)
